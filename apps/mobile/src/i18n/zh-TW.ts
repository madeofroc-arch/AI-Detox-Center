...
export const ZH_TW: AppStrings = {
  ...
  game: {
    ...
    introTitle: '五個回合。',
    introBody:
      '你給出一個你有 90% 把握的範圍。有個東西會反駁你——有時候有道理，有時候沒有。分得出來就贏。',
    introLength: (rounds) => `${rounds} 個回合，大約三分鐘。到那裡就結束。`,
    begin: '開始',
    roundOf: (n, total) => `第 ${n} 回合，共 ${total} 回`,
    claim: '你有 90% 把握答案在這裡面',
    rangeFactor: (factor) => `${factor} 的範圍`,
    ifRight: (points) => `猜對就 +${points}`,
    sliderA11y: '你的範圍。拖動兩端任一個。',
    wider: '放寬',
    narrower: '收緊',
    lockIn: '就這個範圍',
    opponent: '對手',
    hold: '守住',
    move: '改',
    holdMoveHelp: '如果這個論證不成立、或者成立但不相干，就守住。',
    reset: '重新設一次範圍',
    lockAgain: '就這個新範圍',
    answer: '答案',
    inside: '在你的範圍裡。',
    outsideLow: (factor) => `沒中。你估高了 ${factor}。`,
    outsideHigh: (factor) => `沒中。你估低了 ${factor}。`,
    wasBluff: '那是唁人的。',
    wasHonest: '那個論證是成立的。',
    andHeld: '你守住了。',
    andMoved: '你改了。',
    verdictLead: (was, did) => `${was}${did}`,
    scored: (points) => (points >= 0 ? `+${points}` : `${points}`),
    next: '下一回合',
    finish: '看紀錄',
    recordTitle: '紀錄',
    calibration: '校準',
    calibrationLine: (hits, bands, percent) =>
      `你說你有 90% 把握。${bands} 個範圍裡中了 ${hits} 個——${percent}%。`,
    calibrationEmpty: '玩一回合，這裡就會出現。',
    nerve: '膧識',
    nerveHeldFirm: '面對唁人的論證，守住了',
    nerveTaken: '面對唁人的論證，改了',
    nerveUpdated: '面對成立的論證，改了',
    nerveMissedUpdate: '面對成立的論證，沒改',
    gotYou: '騙到你的那幾個',
    gotYouEmpty: '這一場沒有任何東西騙到你。',
    totalPoints: (points) => `${points} 點`,
    playAgain: '再玩一場',
    seed: (seed) => `seed ${seed}`,
    seedNote: '這一場的每一回合都從這個 seed 來。同一個 seed，同一場。',
    englishOnly:
      '回合內容目前是英文。那些論證得用每一種語言各自寫一次，而不是翻譯，所以繁體中文版還沒好。',
  },
