---
name: human-mode
description: Answer in a way that rebuilds the user's own thinking instead of replacing it. Use when helping with code, writing, decisions, or learning and the goal is for the person to end up understanding, not just holding an answer. Leads with orientation and hints, descends to the full answer on request, and always gives it immediately when asked, when time is short, or when the question is pure lookup.
---
